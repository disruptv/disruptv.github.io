/* global disruptvBgColors, disruptvColor, jQuery, wp, _ */
/**
...
(function() {
    ...
    wp.customize('accent_hue', function(value) {
      value.bind(function(to) {
        ...
        Object.keys(disruptvBgColors).forEach(function(context) {
          ...
          if (disruptvBgColors[context].color) {
            backgroundColorValue = disruptvBgColors[context].color;
          } else {
            backgroundColorValue = wp.customize(disruptvBgColors[context].setting).get();
          }
          disruptvSetAccessibleColorsValue(context, backgroundColorValue, to);
        });
      });
    });
    ...
    Object.keys(disruptvBgColors).forEach(function(context) {
      wp.customize(disruptvBgColors[context].setting, function(value) {
        value.bind(function(to) {
          ...
          disruptvSetAccessibleColorsValue(context, to, wp.customize('accent_hue').get(), to);
        });
      });
    });
  });
   ...
   * @return {void}
   */
  function disruptvSetAccessibleColorsValue(context, backgroundColor, accentHue) {
    let value;
    ...
    const colors = disruptvColor(backgroundColor, accentHue);
