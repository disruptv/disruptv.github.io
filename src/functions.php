...
require get_template_directory() . '/classes/class-disruptv-customize.php';

// Require Separator Control class.
require get_template_directory() . '/classes/class-disruptv-separator-control.php';

// Custom comment walker.
require get_template_directory() . '/classes/class-disruptv-walker-comment.php';

// Custom page walker.
require get_template_directory() . '/classes/class-disruptv-walker-page.php';

// Custom script loader class.
require get_template_directory() . '/classes/class-disruptv-script-loader.php';

// Non-latin language handling.
require get_template_directory() . '/classes/class-disruptv-non-latin-languages.php';
 ...
 */
function disruptv_register_styles() {
...
add_action( 'wp_enqueue_scripts', 'disruptv_register_styles' );
 ...
 * @return void
 */
function disruptv_non_latin_languages() {
	$custom_css = disruptv_Non_Latin_Languages::get_non_latin_css( 'front-end' );

	if ( $custom_css ) {
		wp_add_inline_style( 'disruptv-style', $custom_css );
	}
}

add_action( 'wp_enqueue_scripts', 'disruptv_non_latin_languages' );
...
function disruptv_menus() {
	...
	$locations = array(
		'primary'  => __( 'Desktop Horizontal Menu', 'disruptv' ),
		'expanded' => __( 'Desktop Expanded Menu', 'disruptv' ),
		'mobile'   => __( 'Mobile Menu', 'disruptv' ),
		'footer'   => __( 'Footer Menu', 'disruptv' ),
		'social'   => __( 'Social Menu', 'disruptv' ),
	);
 ...
 * @return string $html
 */
function disruptv_get_custom_logo( $html ) {
...
add_filter( 'get_custom_logo', 'disruptv_get_custom_logo' );
 ...
 */
function disruptv_skip_link() {
	echo '<a class="skip-link screen-reader-text" href="#site-content">' . __( 'Skip to the content', 'disruptv' ) . '</a>';
}

add_action( 'wp_body_open', 'disruptv_skip_link', 5 );
 ...
 */
function disruptv_sidebar_registration() {
	...
	register_sidebar(
		array_merge(
			$shared_args,
			array(
				'name'        => __( 'Footer #1', 'disruptv' ),
				'id'          => 'sidebar-1',
				'description' => __( 'Widgets in this area will be displayed in the first column in the footer.', 'disruptv' ),
			)
		)
	);
	...
	register_sidebar(
		array_merge(
			$shared_args,
			array(
				'name'        => __( 'Footer #2', 'disruptv' ),
				'id'          => 'sidebar-2',
				'description' => __( 'Widgets in this area will be displayed in the second column in the footer.', 'disruptv' ),
			)
		)
	);
...
add_action( 'widgets_init', 'disruptv_sidebar_registration' );
 ...
 * @return string $html
 */
function disruptv_read_more_tag( $html ) {
	return preg_replace( '/<a(.*)>(.*)<\/a>/iU', sprintf( '<div class="read-more-button-wrap"><a$1><span class="faux-button">$2</span> <span class="screen-reader-text">"%1$s"</span></a></div>', get_the_title( get_the_ID() ) ), $html );
}

add_filter( 'the_content_more_link', 'disruptv_read_more_tag' );
 ...
 * @return void
 */
function disruptv_customize_controls_enqueue_scripts() {
	$theme_version = wp_get_theme()->get( 'Version' );
...
function disruptv_customize_preview_init() {
	...
	wp_add_inline_script(
		...
		sprintf(
			...
			wp_json_encode( disruptv_customize_opacity_range() )
		)
	);
}

add_action( 'customize_preview_init', 'disruptv_customize_preview_init' );
 ...
 */
function disruptv_get_color_for_area( $area = 'content', $context = 'text' ) {
 ...
 * @return array
 */
function disruptv_get_customizer_color_vars() {
	$colors = array(
		'content'       => array(
 ...
 * @return array
 */
function disruptv_get_elements_array() {
	...
	*/
	return apply_filters( 'disruptv_get_elements_array', $elements );
}
